...
2) Systém ověří kapacity a limity:
   ...
   - pravidlo „více rezervací jednoho obchodníka jen pokud na tiketu není rezervace jiného obchodníka“ (výchozí).
...
- Maximální počet aktivních rezervací na tiket (výchozí 3; nastavuje admin, návrh může dát developer i obchodník u leadu)
- Pravidlo konkurence: obchodník může mít více rezervací na jednom tiketu pouze tehdy, pokud na tiketu nemá rezervaci jiný obchodník (výchozí). Pokud je na tiketu rezervace více obchodníků, výchozí limit je 1 rezervace na obchodníka.
- Čas pro podpis investora (výchozí 48 h; upravitelné adminem)
- Čas pro podpis developera (výchozí 48 h; upravitelné adminem)
- Čas pro jednání po aktivaci (výchozí 30 dní; upravitelné adminem)
- Doba zveřejnění tiketu (publikační okno): výchozí hodnota je nastavena administrátorem (globálně), lze změnit per tiket.
...
- Slot = kapacita obchodníka, kolik aktivních rezervací může mít paralelně.
...
- Pravidlo konkurence (výchozí):
  - pokud na tiketu **není** rezervace jiného obchodníka, může jeden obchodník držet až do plné kapacity tiketu (např. 3 rezervace),
  - pokud na tiketu **je** rezervace jiného obchodníka, výchozí limit je **1 aktivní rezervace na obchodníka**.
...
Kanonická pravidla (podle zadání):
- Platforma odvádí **10 % ze svého podílu** z provize do poolu.
- Pool se vyhodnocuje každých **6 měsíců**.
- Pool se vyplácí obchodníkům podle dosaženého obratu investic (profinancovaných částek) v daném období, pokud splní minimální stanovený obrat.
- Pokud splní minimální obrat první 3 obchodníci:
  - rozdělení poolu 50 % / 30 % / 20 %.
- Pokud nesplní minimální obrat nikdo:
  - 50 % poolu zůstává platformě,
  - 50 % poolu se převádí do dalšího období.

Návrh doplnění (aby bylo implementovatelné – vyžaduje potvrzení):
- Pokud splní limit 1 nebo 2 obchodníci, výplata je „vítěz bere vše“ (100 % pro #1). Druhý v pořadí nedostává nic.
- Počítání obratu: součet profinancovaných částek tiketů, kde je obchodník evidovaný jako obchodník 1 (přivedl investora).
- Tie-break: při shodě rozhoduje dřívější datum profinancování (nebo ruční rozhodnutí administrátora).

Administrace:
- Administrátor nastavuje: minimální obrat, období, procento do poolu, rozdělení výher, pravidla převodu.
...
| Limit aktivních rezervací na tiket pro jednoho obchodníka při konkurenci | integer | ano | administrátor | administrátor | Výchozí 1; platí, pokud na tiketu existuje rezervace jiného obchodníka |
...
| Zůstatek poolu | CZK | ano | systém | administrátor | Průběžná výše poolu |
| Příspěvek z provize | CZK | ano | systém | administrátor | Vzniká při přijetí platby platformě |
| Zdrojový tiket / provize | reference | ano | systém | administrátor | Auditní vazba |
| Hodnocený obrat obchodníka | CZK | ano | systém | administrátor | Součet profinancovaných částek v období (definice k potvrzení) |
| Minimální obrat (kvalifikace) | CZK | ano | administrátor | administrátor | Prahová hodnota pro účast |
| Výherci období | list(broker) | volitelné | systém / administrátor | administrátor | Top1 nebo top3 dle pravidel |
| Rozdělení poolu | procenta | ano | administrátor | administrátor | 100 % nebo 50/30/20 |
...
- Tiket je plný (dosažen max. počet aktivních rezervací) nebo je aktivní konkurence a obchodník už na tiketu rezervaci má.
...
- Default doba zveřejnění tiketu (publikační okno) – návrh: 90 dní (upravitelná)
...
- minimální obrat pro kvalifikaci,
- pravidla výplaty (50/30/20, vítěz bere vše, rollover 50/50 při 0 kvalifikovaných),
- způsob výpočtu obratu (role obchodníka 1 vs role 1+2).
...
1) Pool program
- Jaký je minimální obrat pro kvalifikaci (částka) a platí pro všechny úrovně stejně?
- Počítá se obrat jen pro roli obchodník 1, nebo i obchodník 2?
- Co přesně se stane, pokud se kvalifikují jen 1–2 obchodníci (potvrdit „vítěz bere vše“)?
- Jak řešit shody (tie-break) a reklamace výsledků?

2) Doba zveřejnění tiketů
- Jaká má být výchozí doba zveřejnění (např. 90 dní / 180 dní)?
- Co se má stát s tikety po expiraci (jen skrytí vs archivace) a jaký má být proces re-publikace?

3) Podpisní toky
- Potvrdit, že fyzický podpis řešíme pouze na straně investora (developer podepisuje vždy elektronicky), nebo umožnit fyzický podpis i pro developera.

4) Doklady a audit
- Jaké přílohy budou povinné při ručním potvrzení financování a úhrad (např. potvrzení o převodu)?
