...
export function MyReservationsPageV2() {
  ...
  const stats = useMemo(() => {
    ...
    const completed = mockReservations.filter(r => r.phase === 'SUCCESS');
  ...
  const filteredReservations = useMemo(() => {
    ...
    if (activeTab === 'requiresAction') {
      ...
      filtered = filtered.filter(r => r.phase === 'SUCCESS');
    }
    ...
    filtered.sort((a, b) => {
      if (sortBy === 'newest') {
        ...
        return b.reservedAmount - a.reservedAmount;
      }
      return 0;
    });
      ...
      <div className="space-y-3">
        {filteredReservations.length === 0 ? (
          ...
          filteredReservations.map((reservation) => {
            ...
            const rating = getRatingBadge(reservation.projectId);
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-600">
                          {ticket?.tiparLevel || 'Standard'} · {getPhaseLabel(reservation.phase)}
                        </p>
                      </div>
                      
                      {/* Rating Badge (Investown) */}
                      <span className={`px-2 py-1 ${getRatingColor(rating)} text-xs font-semibold rounded flex-shrink-0`}>
                        {rating}
                      </span>
                    </div>

                    {/* Stats Row (inline, Fingood gray) */}
                    <div className="flex items-center gap-4 text-xs text-gray-600 mb-2">
                      <span className="font-semibold text-gray-900">
                        {reservation.reservedAmount.toLocaleString('cs-CZ')} Kč
                      </span>
                      <span>·</span>
                      <span>{ticket?.expectedYield || 0}% p.a.</span>
                      <span>·</span>
                      <span>{ticket?.duration || 0} měs.</span>
                      {deadlineInfo && (
                        <>
                          <span>·</span>
                          <span className={`flex items-center gap-1 ${deadlineInfo.urgent ? 'text-red-600 font-medium' : ''}`}>
                    ...
                    {/* Progress Bar (Fingood simple, green) */}
                    ...
                    {reservation.phase === 'WAITING_DEVELOPER_DECISION' && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-700">
                          Developer nereaguje několik dní
                        </span>
                      </div>
                    )}
